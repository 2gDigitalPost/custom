...
class TaskEditWdg(BaseRefreshWdg):
    ...
    def get_statuses_select_from_task_pipe(my, status, pipe_code, search_type, task_sk, user_is_scheduler):
        ...
        if my.task_code in ['',None]:
            ...
            if len(tasks_found) > 0:
                ...
                if 'task' not in pipe_code:
                    pipe_code = 'task'
                task_select = my.get_statuses_select_from_task_pipe(task.get_value('status'),pipe_code,'sthpw/pipeline',task.get_search_key(), user_is_scheduler)
...
#        return behavior
    ...
    def get_deactivate_behavior(my, title_code):
        behavior = {'css_class': 'clickme', 'type': 'click_up', 'cbjs_action': '''        
                        try{
                          var top_el = spt.api.get_parent(bvr.src_el, '.twog_order_builder');
                          title_code = '%s'; 
    ...
    def get_set_redo(my, title_code, order_sk):
        behavior = {'css_class': 'clickme', 'type': 'click_up', 'cbjs_action': '''        
                        try{
                          var top_el = spt.api.get_parent(bvr.src_el, '.twog_order_builder');
