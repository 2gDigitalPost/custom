...
class IncompleteWOWdgInnerds(BaseTableElementWdg):
    ...
    def select_all_behavior(my, parent_view, code):
        behavior = {'css_class': 'clickme', 'type': 'change', 'cbjs_action': '''        
                try{
                    ...
                    assigned = '';
                    hours = '';
                    colors={'Pending':'#d7d7d7','In Progress':'#f5f3a4','In_Progress':'#f5f3a4','On Hold':'#e8b2b8','On_Hold':'#e8b2b8','Client Response':'#ddd5b8','Completed':'#b7e0a5','Need Buddy Check':'#e3701a','Ready':'#b2cee8','Internal Rejection':'#ff0000','External Rejection':'#ff0000','Rejected':'#ff0000','Fix Needed':'#c466a1','Failed QC':'#ff0000','DR In_Progress':'#d6e0a4','BATON In_Progress':'#c6e0a4','Export In_Progress':'#796999','Buddy Check In_Progress':'#1aade3'};
                    if(selval){
    ...
    def get_display(my):
        ...
        if 'show_all' in my.kwargs.keys() or parent_view in ['bigboard_hotlist','bigboard_hl','ncr_titles_hl','titles_hl','all_titles_hotlist']:
            show_all = my.kwargs.get('show_all')
            if parent_view in ['titles_hl','bigboard_hl','ncr_titles_hl']:
                show_all = True
        ...
        for task in tasks:
            ...
            status_pull = '''<select name="status_%s" old="%s" id="status_%s" style="background-color: %s; border-radius: 5px;" onChange="colors={'Pending':'#d7d7d7','In Progress':'#f5f3a4','In_Progress':'#f5f3a4','On Hold':'#e8b2b8','On_Hold':'#e8b2b8','Client Response':'#ddd5b8','Completed':'#b7e0a5','Need Buddy Check':'#e3701a','Ready':'#b2cee8','Internal Rejection':'#ff0000','External Rejection':'#ff0000','Rejected': '#ff0000','Fix Needed':'#c466a1','Failed QC':'#ff0000','DR In_Progress': '#d6e0a4', 'BATON In_Progress': '#c6e0a4', 'Export In_Progress': '#796999', 'Buddy Check In_Progress': '#1aade3'};top_el=document.getElementById('inc_row_%s'); top_el.setAttribute('bgcolor','#909977'); me_stat=top_el.getElementById('status_%s');me_stat.style.backgroundColor=colors[me_stat.value];">''' % (sk, status, sk, my.stat_colors[status], sk, sk)
            for val in ['Pending','Ready','On_Hold','Client Response','Fix Needed','Rejected','In_Progress','DR In_Progress','BATON In_Progress','Export In_Progress','Need Buddy Check','Buddy Check In_Progress','Completed']:
                selected = ''
                if status == val:
                    selected = 'selected'
