...
class NewDepartmentTask(BaseRefreshWdg):
    def get_display(self):
        ...
        task_utils.get_assigned_login_group_widget(outer_div)
    ...
    def submit_button_behavior():
        behavior = {
            ...
            'cbjs_action': '''
try {
    var submit_form = function(values) {        spt.api.app_busy_show('Saving...');
        ...
        var assigned_login_group = values.assigned_login_group;
        var due_date = values.due_date;

        // Set up the object for the new task.
        var new_department_task = {
            'process': process,
            'description': description,
            'status': 'Pending',
            'priority': priority,
            'assigned_login_group': assigned_login_group,
        ...
        server.insert('sthpw/task', new_department_task, {'triggers': false});
